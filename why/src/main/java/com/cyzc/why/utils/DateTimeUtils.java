...
public class DateTimeUtils {
    ...
    public final static String KEY_FORMAT4 = "yyyy-MM";
    ...
    public static String getLastMonthStr(Date date, int count) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
        calendar.add(Calendar.MONTH, count); // 得到月份，count为负时是前几月，为正时是后几月
        Date lastDate = calendar.getTime();
        return parseDateTime(lastDate, KEY_FORMAT4);
    }
     ...
     * @return
     */
    public static Date getYearShift(String date, int shift) {
        String targetDate = "";
        String dstr[] = StringUtils.convertStringToArray(date, "-");
        int year = Integer.valueOf(dstr[0]);
        year += shift;
        targetDate = year + "-" + dstr[1] + "-" + dstr[2];
    ...
    // 输入yyyy-MM-dd格式的日期获取dd-1日期
    public static String getPreDay(String dateString) {
        String[] date = dateString.split("-");
        int nextDate = Integer.parseInt(date[2]) - 1;
        String next = String.valueOf(nextDate);
        if (next.length() == 1) {
            next = "0" + next;
        }
        date[2] = next;
        return StringUtils.convertArrayToString(date, "-");
    }
    ...
    // 根据身份证获取出生日期
    public static Date getBirthByccId(String ccId) {
        if (ccId != null) {
            ccId = ccId.replaceAll("\\s*", "");
            if (ccId.length() == 18) {
                String year = ccId.substring(6, 10);
                String month = ccId.substring(10, 12);
                String day = ccId.substring(12, 14);
                return formatDateTime(year + "-" + month + "-" + day,
                        KEY_FORMAT2);
            } else {
                return null;
            }
    ...
    public static String getTimeStrShift(Date date, int i) {
        String time = getTimeStr(date, KEY_FORMAT1);
        Date dateTime = DateTimeUtils.getDateHourShift(time, i);
        String timeString = getTimeStr(dateTime, KEY_FORMAT1);
        return timeString;
    }
     ...
     * @return
     */
    public static Date trueEndDate(Date date) {
    ...
    /**
     * 两个时间相差距离多少年 不足一年默认为一年
     * @param str1 时间参数 1 格式：1990-01-01 12:00:00
     * @param str2 时间参数 2 格式：2009-01-01 12:00:00
     * @return String 返回值为：xx天xx小时xx分
     */
    public static long getDistanceYear(String str1, String str2) {
        DateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        Date one;
        Date two;
        long year = 0;
    ...
    public static Date getFirstDayOfMouthDate() {
        DateTimeFormatter time = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
        LocalDateTime now = LocalDateTime.now();
        LocalDateTime beginDateTime = now.with(TemporalAdjusters.firstDayOfMonth()).withHour(0)
                .withMinute(0).withSecond(0);
        return Date.from( beginDateTime.atZone( ZoneId.systemDefault()).toInstant());
    }
    public static Date getLastDayOfMouthDate() {
        DateTimeFormatter time = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
        LocalDateTime now = LocalDateTime.now();
        LocalDateTime endDateTime = now.with(TemporalAdjusters.lastDayOfMonth()).withHour(23)
